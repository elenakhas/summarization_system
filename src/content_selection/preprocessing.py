...
def preprocess_data(nlp, data):
    ...
    for topic_id in data.keys():
        ...
        for doc in docs:
            doc = nlp(doc)
            spacydocs.append(get_sents_noquest(doc))
        documents[topic_id] = spacydocs
    return documents

def process_document(spacydoc, doc_id, topic_id, sentences):
    ...
    for index, sentence in enumerate(spacydoc):
        sentence_info = {"doc_id": doc_id, "topic_id": topic_id, "index": index, "lemmas": list(), "length": 0, "total_sent": total_sents}
        # all tokens
